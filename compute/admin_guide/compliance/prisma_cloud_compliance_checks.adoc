...
For this check, we use the database composed of our Intelligence Stream (IS) and WildFire. We check the CI images with the IS and WildFire, and deployed images only with the IS. Users can populate md5 by adding them to the custom feed/malware and those will be added and checked as well.
...
We use pattern checks with various terms related to secrets, and no extra analysis is done on these environment variables.
The search can detect private keys, Amazon access keys (for example `AKIAIOSFODNN7EXAMPLE`), and key-values secrets with known keys (for example "token=fdfsfgsdfkjbsdkf").

*425 -- Private keys stored in image (image and serverless function check)*::

Opens the file with an image or serverless function, and use a regex pattern to search for private keys alone.
...
We improved the search logic to detect binaries with known crypto miners.
Some of the examples from Docker Hub that includes the crypto miners that we identify are:
